...
public class OrdemServicoIntegrationTests(CustomWebApplicationFactory<Program> factory) : IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    private async Task<(Guid OsId, Guid OsServicoId)> PrepararOsEmExecucaoAsync()
    {
        ...
        await _client.PatchAsync($"/api/ordens-servico/{os.Id}/iniciar-diagnostico", null);

        var dtoServico = new AdicionarServicoDTO(deps.ServicoId, 150.00m);
        await _client.PostAsJsonAsync($"/api/ordens-servico/{os.Id}/servicos", dtoServico);

        await _client.PatchAsync($"/api/ordens-servico/{os.Id}/enviar-para-aprovacao", null);

        await _client.PatchAsync($"/api/ordens-servico/{os.Id}/aprovar", null);

        var osDetalhe = await _client.GetFromJsonAsync<OrdemServicoDetalheDTO>($"/api/ordens-servico/{os.Id}");
    ...
    public async Task CreateOrdemServico_ComDadosValidos_DeveRetornarCreated()
    {
        ...
        var response = await _client.PostAsJsonAsync("/api/ordens-servico", requestDto);
    ...
    public async Task GetOrdemServicoById_QuandoExiste_DeveRetornarOk()
    {
        ...
        var response = await _client.GetAsync($"/api/ordens-servico/{osCriada.Id}");
    ...
    public async Task GetAllOrdensServico_DeveRetornarOkEListaPaginada()
    {
        ...
        var response = await _client.GetAsync("/api/ordens-servico?pageNumber=1&pageSize=10");
    ...
    public async Task AdicionarServico_QuandoOSStatusValido_DeveRetornarNoContent()
    {
        ...
        var response = await _client.PostAsJsonAsync($"/api/ordens-servico/{osCriada.Id}/servicos", dto);
    ...
    public async Task AdicionarInsumo_QuandoOSStatusValido_DeveRetornarNoContent()
    {
        ...
        var response = await _client.PostAsJsonAsync($"/api/ordens-servico/{osCriada.Id}/insumos", dto);
    ...
    public async Task IniciarDiagnostico_QuandoStatusRecebida_DeveRetornarNoContent()
    {
        ...
        var response = await _client.PatchAsync($"/api/ordens-servico/{osCriada.Id}/iniciar-diagnostico", null);
    ...
    public async Task AguardarAprovacao_AposDiagnosticoEServico_DeveRetornarNoContent()
    {
        ...
        await _client.PatchAsync($"/api/ordens-servico/{os.Id}/iniciar-diagnostico", null);
        await _client.PostAsJsonAsync($"/api/ordens-servico/{os.Id}/servicos", new AdicionarServicoDTO(deps.ServicoId, 100m));

        var response = await _client.PatchAsync($"/api/ordens-servico/{os.Id}/enviar-para-aprovacao", null);
    ...
    public async Task IniciarServico_QuandoOsEmExecucao_DeveRetornarNoContent()
    {
        ...
        var response = await _client.PatchAsync($"/api/ordens-servico/{osId}/servicos/{osServicoId}/iniciar", null);
        var erro = await response.Content.ReadAsStringAsync();
    ...
    public async Task ConcluirServico_AposIniciado_DeveRetornarNoContent()
    {
        ...
        await _client.PatchAsync($"/api/ordens-servico/{osId}/servicos/{osServicoId}/iniciar", null);

        var response = await _client.PatchAsync($"/api/ordens-servico/{osId}/servicos/{osServicoId}/concluir", null);
    ...
    public async Task Entregar_SeNaoFinalizada_DeveRetornarBadRequest()
    {
        ...
        var response = await _client.PatchAsync($"/api/ordens-servico/{os.Id}/entregar", null);
